...
Design tradeoffs (must decide early)
	•	Parallel vs sequential MACs:
	•	Parallel (one MAC per tap): max throughput, heavy DSP usage, consumes LUT/DSP/FF.
	•	Sequential (one MAC reused across taps): low resource, higher latency, lower throughput.
	•	AXI data path:
	•	AXI-Stream + DMA = low latency and continuous streaming.
	•	Memory-mapped block processing = easier for block transfer, less PL streaming logic.
	•	Coefficient update method:
	•	On-chip BRAM preload via AXI-Lite writes, or point to DDR and fetch via AXI master.

⸻

Cora Z7: suitability & constraints
	•	Cora Z7 is Zynq-7000 based (variants XC7Z007S/XC7Z010); it has a Cortex-A9 PS, DDR3, and FPGA fabric with DSP slices and block RAM. It’s supported by Vivado/Vitis.  ￼
	•	Implication:
	•	You can implement 8–64 tap FIR accelerators. For wide parallel implementations (64-tap, fully parallel), verify available DSP48 slices (Cora Z7 has limited DSPs; you might need a sequential or partially parallel design). Check your specific variant’s DSP count (reference manual).  ￼
	•	Use AXI DMA or AXI-Stream IP cores available in Vivado to connect PS DDR and custom FIR IP.

⸻

How to achieve a 4× performance gain (practical guidance)
	1.	Define baseline: implement same FIR in C (fixed-point) running on Zynq PS CPU (ARM Cortex-A9), measure runtime for a fixed block (e.g., 100k samples).
	2.	Implement PL accelerator to process data in streaming fashion with DMA (double buffering). Use clock frequency ~100–150 MHz in PL.
	3.	Overlap DMA transfers and PL processing; maximize PL utilization (no stalls).
	4.	Use proper fixed-point widths and avoid unnecessary sign-extensions.
	5.	If fully parallel MACs are not feasible due to DSP limits, implement pipelined multi-MAC (e.g., 8 MACs in parallel and loop to cover 64 taps) to balance throughput and resources.

A measured 4× speedup is realistic if the baseline is single-threaded C on Cortex-A9 and the accelerator processes data with pipeline & DMA (you’ll need to show measurements: cycles or wall time).

⸻

Verification & performance measurement checklist
	•	Functional correctness: compare outputs bit-exact (or within acceptable error) vs golden model.
	•	Latency: cycles from input sample arrival to output availability.
	•	Throughput: samples/sec and sustained rates.
	•	Resource usage: LUTs, FFs, BRAM, DSP48 usage from Vivado synthesis.
	•	Timing: can accelerator meet your target clock? (Focus on meeting 100–200 MHz in PL).
	•	Power (optional): check board power if needed.
Practical tips & gotchas
	•	Simulate fixed-point overflow: implement saturation if needed.
	•	Use -generic parameters to make IP re-usable.
	•	Use Vivado HLS if you prefer C → hardware flow (can be faster to prototype but less control on resource mapping).
	•	Watch BRAM and DSP budget on Cora Z7; do partial parallelization if needed.
	•	Use AXI DMA IP (from Vivado IP catalog) to minimize custom data-movement logic.
	•	Use performance counters / gettimeofday in Linux to measure wall time accurately.
